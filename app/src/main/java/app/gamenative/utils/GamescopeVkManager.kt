...
object GamescopeVkManager {
    ...
    // Match GameHub's live layout. GameScopeVK mmaps this 10-byte file.
    private const val CONTROL_FILE_RELATIVE_PATH = "usr/etc/gamescope.control"
    ...
    const val EXTRA_USE_CUSTOM_DRIVER = "gamescopeVkUseCustomDriver"
    ...
    private const val ENV_SURFACE_BGRA = "GAMESCOPE_SURFACE_USING_BGRA"
    ...
    fun useCustomDriver(container: Container): Boolean =
        container.getExtra(EXTRA_USE_CUSTOM_DRIVER, "false").equals("true", ignoreCase = true)
    ...
    fun applyLaunchEnv(container: Container, envVars: EnvVars): Boolean {
        ...
        // Match GameHub's surface format selection. This is required for BGRA DRI3/
        // DirectRendering paths observed in the live GameHub environment.
        envVars.put(ENV_SURFACE_BGRA, "1")

        // GAMESCOPE_DRIVER_PATH: real GPU driver that GameScopeVK will wrap.
        // Be conservative by default: WinNative showed that arbitrary app-data driver
        // paths can be rejected by Android's sphal linker namespace. Leave the var
        // unset unless an explicit experiment opt-in is enabled for the container.
        if (useCustomDriver(container)) {
            findDriverSo(rootDir, envVars)?.let { driverPath ->
            ...
            envVars.remove(ENV_DRIVER_PATH)
            Timber.tag(TAG).w("Custom downstream driver disabled; GameScopeVK will auto-detect system Vulkan")
        }
    ...
    private fun writeControlFileRaw(
        ...
        return try {
            ...
            map.put(1)                                   // native rendering mode: DirectRendering/BGRA (matches GameHub)
     ...
     */
    private fun findDriverSo(rootDir: File, envVars: EnvVars): String? {
        val selectedDriverPath = envVars.get("ADRENOTOOLS_DRIVER_PATH")
        val selectedDriverName = envVars.get("ADRENOTOOLS_DRIVER_NAME")
        if (!selectedDriverPath.isNullOrEmpty() && !selectedDriverName.isNullOrEmpty()) {
            val selected = File(selectedDriverPath, selectedDriverName)
            if (selected.isFile) return selected.absolutePath
        }

        // Fallback: look in the app files dir for an adrenotools driver.
