...
private fun calculateInstalledCount(context: android.content.Context, state: LibraryState): Int {
    ...
    val itchCount = if (state.showItchInLibrary && ItchService.hasStoredCredentials(context)) {
    ...
    return steamCount + customGameCount + gogCount + epicCount + itchCount + amazonCount
}
    ...
    Scaffold(
        ...
        Box(
            ...
            val totalSkeletonCount = remember(
                ...
                val itchInstalledCount = if (state.showItchInLibrary && ItchService.hasStoredCredentials(context)) PrefManager.itchInstalledGamesCount else 0
                ...
                val total = customCount + steamCount + gogInstalledCount + epicInstalledCount + itchInstalledCount + amazonInstalledCount
                Timber.tag("LibraryListPane").d("Skeleton calculation - Custom: $customCount, Steam: $steamCount, GOG installed: $gogInstalledCount, Epic installed: $epicInstalledCount, Itch installed: $itchInstalledCount, Amazon installed: $amazonInstalledCount, Total: $total")
