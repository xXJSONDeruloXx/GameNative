...
Device Device::create() {
    ...
    appInfo.pApplicationName   = "GNFramegen";
    ...
    for (const char* ext : kRequiredDeviceExts) {
        if (hasExt(availExts, ext))
            ...
            GNFG_LOGW("Device ext not available: %s", ext);
    }
    ...
    GNFG_LOGI("GN device created: phys=%p dev=%p computeQF=%u", (void*)phys, (void*)dev, computeQF);
    return d;
}
