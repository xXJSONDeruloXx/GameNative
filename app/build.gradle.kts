...
val androidNdkDir = androidSdkDir?.let { file("$it/ndk/$androidNdkVersion") }
...
fun registerWinlatorBuildTask(taskName: String, abi: String) = tasks.register(taskName) {
    ...
    outputs.file(sourceBuiltWinlatorDir.map { it.file("$abi/libwinlator.so") })
    ...
    doLast {
        ...
        exec {
            commandLine("cmake", "--build", buildDir.absolutePath, "--target", "winlator")
        }
    }
}
...
val stageSourceBuiltWinlatorJniLibs = tasks.register<Sync>("stageSourceBuiltWinlatorJniLibs") {
    ...
    from("src/main/jniLibs") {
        exclude("arm64-v8a/libwinlator.so", "armeabi-v7a/libwinlator.so")
    }
